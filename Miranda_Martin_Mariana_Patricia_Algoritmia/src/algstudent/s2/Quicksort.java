...
public class Quicksort {
	...
	public static int median_of_three(int[] a, int left, int right) { 
		...
		if (a[left] > a[center])
			Vector.swap(a, left, center);
		if (a[left] > a[right])
			Vector.swap(a, left, right);
		if (a[center] > a[right])
			Vector.swap(a, center, right);
		return center;
	}
